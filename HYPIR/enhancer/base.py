...
class BaseEnhancer:
    ...
    def enhance(
        ...
        prompt: str,
        upscale: int = 1,
        encode_patch_size: int = 512,
        decode_patch_size: int = 512,
        return_type: Literal["pt", "np", "pil"] = "pt",
    ) -> torch.Tensor | np.ndarray | List[Image.Image]:
        patch_size = encode_patch_size
        ...
        z = make_tiled_fn(
            ...
            size=encode_patch_size // vae_scale_factor,
            stride=encode_patch_size // 2 // vae_scale_factor,
            progress=True,
        ...
        with enable_tiled_vae(
            self.vae,
            is_decoder=True,
            tile_size=decode_patch_size // vae_scale_factor,
            dtype=self.weight_dtype,
        ):
